export function getUniqueWaysToClimbStaircase(n: number) {
  if (n === 1) {
    return 1;
  }
  if (n === 2) {
...
export function getUniqueWaysToClimbStaircase_Iterative(n: number) {
  if (n < 1) {
    return null;
  }
...
export function getUniqueWaysToClimbStaircase_Advanced(
  n: number,
  stepsizes: number[]
) {
